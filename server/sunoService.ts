...
interface GenerateSongParams {
  ...
  occasion?: string;
  tone: string;
  genre?: string; // we will normalize this to black-gospel when needed
  interests?: string;
  insideJokes?: string;
  customLyrics?: string;
  customTitle?: string;
...
/**
 * Normalize user genre inputs to a strict internal set.
 * Any "gospel-like" genre gets forced to "black-gospel" to ensure authentic sound.
 */
...
async function pollTaskStatus(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    const { status, response: taskResponse, errorMessage } =
      response.data.data;
    lastStatus = status;
    lastError = errorMessage;
...
async function concatenateClips(clipIds: string[]): Promise<string> {
  ...
  const finalTrack = result.sunoData[0];
  console.log(
    `[Suno Concat] Final song duration: ${finalTrack.duration}s`
  );
...
/**
 * Generate song with provided lyrics (no OpenAI lyrics generation) - Extended version (~3 minutes).
 * This version is tuned for authentic Black gospel when the genre is gospel-like.
 */
export async function generateSongWithLyrics(params: {
  title: string;
  lyrics: string;
  ...
  try {
    ...
    // Normalize the genre first (forces gospel-ish → black-gospel)
    ...
    if (styleMatch && styleMatch[1]) {
      ...
      style = getDetailedStyle(normalizedGenre, params.tone);
      console.log(`[Suno] Using auto-generated style: ${style}`);
    }

    // Step 1: Generate initial clip (uses V4 for longer initial output)
    console.log(
      `[Suno] Starting extended song generation (~3 minutes) for: ${params.title} [genre=${normalizedGenre}]`
    );
    ...
    while (currentDuration < targetDuration && extensionCount < maxExtensions) {
      ...
      try {
        const extension = await extendSong({
          audioId: currentAudioId,
          continueAt,
          // Explicitly remind the model this is a Black gospel worship song
          prompt: `Continue this BLACK GOSPEL worship song with the same church choir energy, Hammond organ, and call-and-response feel. Use similar themes and flow as these lyrics: ${params.lyrics.slice(
            0,
            200
          )}...`,
          style,
    ...
    console.log(
      `[Suno] Extended song generation completed! Final URL ready.`
    );
 ...
 *  - calls OpenAI to generate lyrics and then passes them into the Black-gospel-aware generator.
 */
export async function generateSong(
  ...
  // Normalize incoming genre right away so everything downstream is gospel-aware.
  const normalizedGenre = normalizeGenre(params.genre);

  // If custom lyrics provided, use them directly
  if (params.customLyrics && params.customTitle) {
    console.log(
      `[Suno] Using custom lyrics provided by user for: ${params.customTitle} [genre=${normalizedGenre}]`
    );
    return generateSongWithLyrics({
      title: params.customTitle,
      lyrics: params.customLyrics,
  ...
  // Tell OpenAI that genre is black-gospel when appropriate
  const songLyrics = await generateSongLyrics({
    recipientName: params.recipientName,
    relationship: params.relationship,
    occasion: params.occasion,
    tone: params.tone,
    genre: normalizedGenre, // will be "black-gospel" for anything gospel-ish
    interests: params.interests,
    insideJokes: params.insideJokes,
  });
